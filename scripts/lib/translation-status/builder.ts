...
export class TranslationStatusBuilder {
	...
	getPageUrl({ type = 'blob', refName = 'main', lang, subpath, query = '' }: { type?: string; refName?: string; lang: string; subpath: string; query?: string }) {
		const noDotSrcDir = this.pageSourceDir.replace(/^.\//, '');
		return `https://github.com/${this.githubRepo}/${type}/${refName}` + `/${noDotSrcDir}/${lang}/${subpath}${query}`;
	}
	...
	renderHtmlStatusPage(statusByPage: PageTranslationStatus[], prs: PullRequest[]) {
		// Load HTML template
		const templateFilePath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'template.html');
		...
		return html
			.replace('<!-- TranslationStatusByLanguage -->', this.renderTranslationStatusByLanguage(statusByPage))
			.replace('<!-- TranslationNeedsReview -->', this.renderTranslationNeedsReview(prs))
			.replace('<!-- TranslationStatusByPage -->', this.renderTranslationStatusByPage(statusByPage));
	}
	...
	renderTranslationStatusByLanguage(statusByPage: PageTranslationStatus[]) {
		...
		this.targetLanguages.forEach((lang) => {
				...
				lines.push(
					...outdated.map(
						(content) =>
							...
							`(${this.renderLink(content.translations[lang].githubUrl, 'outdated translation')}, ${this.renderLink(
			...
			if (missing.length > 0) {
				...
				lines.push(`<ul>`);
				lines.push(
					...missing.map((content) => `<li>` + `${this.renderLink(content.githubUrl, content.subpath)} &nbsp; ${this.renderCreatePageButton(lang, content.subpath)}` + `</li>`)
				);
				lines.push(`</ul>`);
			}
			lines.push(`</details>`);
	...
	renderTranslationStatusByPage(statusByPage: PageTranslationStatus[]) {
		...
		lines.push('<tbody>');
		const spacer = `<tr class="spacer">\n${this.targetLanguages.map(() => `<td></td>`).join('\n')}\n</tr>`;
		lines.push(spacer);
		statusByPage.forEach((content) => {
			...
			cols.push(
				...this.targetLanguages.map((lang) => {
					...
					if (translation.isMissing) return `<span title="${lang}: Missing"><span aria-hidden="true">❌</span></span>`;
					if (translation.isOutdated) return `<a href="${translation.githubUrl}" title="${lang}: Needs updating"><span aria-hidden="true">🔄</span></a>`;
	...
	renderCreatePageButton(lang: string, filename: string): string {
		...
		const createUrl = new URL(`https://github.com/${this.githubRepo}/new/main/src/content/docs/${lang}`);
	 ...
	 */
	renderProgressBar(total: number, outdated: number, missing: number, { size = 20 }: { size?: number } = {}) {
