...
const errorURL = 'https://raw.githubusercontent.com/withastro/astro/main/packages/astro/src/core/errors/errors-data.ts';
...
export async function run() {
	...
	for (const comment of astroErrorData.jsdoc) {
		...
		const errorTitle = sanitizeString(astroErrorData.errors[comment.meta.code.name].title ?? comment.name);
		...
		const completeReferenceEntry = [
			...
			getMessage(comment.name, errorCode, astroErrorData.errors[comment.name].message, comment.tags.find((tag) => tag.title === 'message')?.value),
		...
		fs.writeFileSync(`src/content/docs/en/reference/errors/${fileName}.mdx`, getErrorReferenceEntryHeader(errorTitle) + completeReferenceEntry);

		// Build string for error reference list
		astroResult += [`- [**${comment.name}**](/en/reference/errors/${fileName}/) (E${padCode(errorCode)})<br/>${errorTitle}\n`].filter((l) => l !== undefined).join('\n');
	}

	fs.writeFileSync('src/content/docs/en/reference/error-reference.mdx', HEADER + astroResult + FOOTER, 'utf8');
	...
	function getDeprecatedText(deprecateMention) {
		...
		return [``, ':::caution[Deprecated]', typeof deprecateMention === 'string' ? deprecateMention : 'This error cannot be emitted by Astro anymore', ':::'].join('\n');
	}
...
async function getAstroErrorsData() {
	 ...
	 */
	const jsDocComments = jsdoc.explainSync({ source: compiledResult }).filter((data) => data.tags && data.tags.some((tag) => tag.title === 'docs'));
