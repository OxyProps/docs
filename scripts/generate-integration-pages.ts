...
import type { Blockquote, Definition, HTML, Link, ListContent, Paragraph, PhrasingContent, Root, Text } from 'mdast';
...
<DontEditWarning/>\n\n` + readme;
	...
	async #writeReadme(packageName: string, readme: string): Promise<void> {
		...
		return await fs.promises.writeFile(`src/content/docs/en/guides/integrations-guide/${unscopedName}.mdx`, readme, 'utf8');
	}
...
function replaceAsides() {
	return function transform(tree: Root) {
		visit(tree, 'blockquote', (node: Blockquote | Paragraph) => {
			...
			openingParagraph.children = [openAside, trailingText, ...children, closeAside] as ListContent[];
		});
	};
}
...
function removeTOC() {
	return function transform(tree: Root) {
		remove(tree, (node) => {
			...
			return firstItemContent.children.some((child: PhrasingContent) => child.type === 'link' && (child.url.startsWith('#why') || child.url.startsWith('#installation')));
		});
	};
}
