...
async function setDiscordMessage() {
	...
	const list = toTranslate
		.map((s) => {
			const langs =
				Object.keys(s.translations).filter((l) => s.translations[l]?.isMissing || s.translations[l]?.isOutdated).length === Object.keys(languages).length - 1
					? ['all']
					: Object.keys(s.translations);
			return `- [\`${s.subpath}\`](<https://github.com/withastro/docs/tree/main/${join(PAGE_SOURCE_DIRECTORY, 'en', s.subpath)}>) (${
				langs[0] === 'all'
					? 'all'
					: langs
							.filter((lang) => {
