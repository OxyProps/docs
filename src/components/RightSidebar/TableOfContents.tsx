...
const TableOfContents = ({ toc = [], labels, isMobile }: Props) => {
	...
	const Container = ({ children }: { children: ComponentChildren }) => {
		return isMobile ? (
			<details {...{ open }} onToggle={(e: JSX.TargetedEvent<HTMLDetailsElement>) => setOpen(e.currentTarget.open)} className="toc-mobile-container">
				{children}
			</details>
		) : (
			<>{children}</>
		...
		return isMobile ? (
			<summary className="toc-mobile-header">
				<div className="toc-mobile-header-content">
					...
					{!open && currentHeading?.slug !== 'overview' && <span className="toc-current-heading">{unescape(currentHeading?.text || '')}</span>}
				</div>
			</summary>
		) : (
			children
	...
	const TableOfContentsItem = ({ heading }: { heading: TocItem }) => {
		...
		return (
			<li>
				<a className={`header-link depth-${depth} ${currentHeading.slug === slug ? 'current-header-link' : ''}`.trim()} href={`#${slug}`} onClick={onLinkClick}>
					{unescape(text)}
				</a>
				{children.length > 0 ? (
					<ul>
